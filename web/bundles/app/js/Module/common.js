...
var showDefaultModal = function (title, body, footer, width, height, closable) {
    ...
    if (title == '') {
        modalBox.find('.modal-header').hide();
    }
    ...
    if (title == '' && footer == '') {
        modalBox.find('.modal-body').css('padding', '0')
        modalBox.find('.modal-body').find('p').css('margin', '0')
    }
    modalBox.modal('show');
...
function progressBarSuivi(progressId, intervalTimer, progressContainer, hintRendering, completed) {
    ajaxCall({
            url: Routing.generate('progress', {'progression': progressId}),
            dataType: 'json'
        }, function (data) {
...
function initWeatherIcon() {
    $(".weather-icon").attr('data-placement', 'top');
    $(".weather-icon").attr('data-toggle', 'tooltip')
    $(".weather-icon.snow").attr('title', Translator.trans('weather.snow'));
    $(".weather-icon.rain").attr('title', Translator.trans('weather.rain'));
    $(".weather-icon.sun").attr('title', Translator.trans('weather.sun'));
    $(".weather-icon.cloud").attr('title', Translator.trans('weather.cloud'));

    $(".weather-icon").tootltipBootstrap();
}
$(document).on('mouseenter', '.weather-icon', function () {
    initWeatherIcon();
});
...
$('a.workflow_start').on('click', function (e) {
    e.preventDefault();
    var url = $(this).attr('href');
    var name = $(this).text();
    bootbox.confirm({
        title: Translator.trans("workflow_confirm_title",{'name' : name}),
        message: Translator.trans("workflow_confirm_text"),
        buttons: {
            cancel: {
            ...
            confirm: {
                ...
                className: 'btn btn-validate btn-icon btn-validate'
            }
        },
...
$('#restaurant-switcher').on('change', function() {
    var header = Translator.trans('switch_restaurant');
    var content = '<h3>'+Translator.trans('switch_restaurant_confirm')+'</h3>';
    var footer = '<div class="row" style="margin-bottom: 0px;">'+
        ...
        '<button class="btn waves-effect waves-light blue" type="button" id="confirm-switch-restaurant" style="margin-left: 10px;">' +
...
$(document).on('click', '#confirm-switch-restaurant', function(){
     $("#switch-restaurant-form").submit();
});
