...
$(function () {
   ...
   /* var keyUpNamespace = 'check_quick_qty_input';
    $(document).on('focus', '.check_quick_qty_input', function () {
        var self = $(this);
        var e = shortcutController.add(KEY_ENTER, null, null, function () {
            var nextCell = $($($(self).closest('.row').next('.row')[0]).find('.check_quick_qty_input')[0]);
            nextCell.focus();
            nextCell.select();
        }, keyUpNamespace);
    ...
    $(document).on('click', '.addCheckQuickValue', function() {
        ...
        $('#chest_count_smallChest_checkQuickCounts_'+ nbr + '_checkName').val(realcheckName);
    });
