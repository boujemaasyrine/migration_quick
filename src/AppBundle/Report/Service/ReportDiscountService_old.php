...
class ReportDiscountService
{
    ...
    public function getDiscountList($filter)
    {
        ...
        $invoice = $filter['InvoiceNumber'];
            ...
            $sql = "
              ...
              sum( ABS(ticket_line.discount_ttc)) AS discount_ttc,
               (SUM (ticket_line.totalttc)) as total_ttc,
              (sum( ABS(ticket_line.discount_ttc)) / nullif(SUM (ticket_line.totalttc),0)) AS discount_percentage
              FROM ticket
            ...
            $sql .= " GROUP BY ticket.invoicenumber, ticket.enddate, quick_user.first_name, quick_user.last_name , quick_user.matricule, ticket.totalttc, ticket.date ";
    ...
    public function serializeList($data, $filter)
    {
        ...
        foreach ($data as $element) {
            ...
            $tmpLine['amount'] = $element['total_ttc'];
            ...
            if (is_null($filter['discountPerCentMin']) && is_null($filter['discountPerCentMax'])) {
                ...
                if (is_null($filter['discountPerCentMin']) && !is_null($filter['discountPerCentMax'])) {
                    if (floatval($element['discount_percentage']) <= floatval($filter['discountPerCentMax']/100)) {
                        $list[] = $tmpLine;
                    }
                } else {
                    if (!is_null($filter['discountPerCentMin']) && is_null($filter['discountPerCentMax'])) {
                        if (floatval($element['discount_percentage']) >= floatval($filter['discountPerCentMin']/100)) {
                            $list[] = $tmpLine;
                        }
                    } else {
                        if (floatval($element['discount_percentage']) >= floatval($filter['discountPerCentMin'] /100) && floatval($element['discount_percentage']) <= floatval($filter['discountPerCentMax']/100)) {
                            $list[] = $tmpLine;
                        }
                    }
                }
    ...
    public function generateExcelFile($result, $filter, Restaurant $currentRestaurant, $logoPath)
    {
        ...
        foreach ($result['report'] as $line) {
            ...
            ExcelUtilities::setFormat($sheet->getCell('A'.$i),\PHPExcel_Cell_DataType::TYPE_NUMERIC);
