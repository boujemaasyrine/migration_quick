...
class ReportCashBookService
{
        ...
        while ($date <= $criteria['endDate']) {
            ...
            if ($closure) {
                ...
                $result[$date->format('Y-m-d')] = [
                    ...
                    'recipeTicketsAmount' => $cashBookReport->getTotalRecipeTicketsByDate(),
                    'differenceRecipeExpense' => $cashBookReport->getDifferenceRecipeExpense(),
                    'currentChestAmount' => $this->getCurrentChestAmount($cashBookReport),
    ...
    public function createExcelFileSecondVersion($result, $criteria, Restaurant $currentRestaurant, $logoPath)
    {
        ...
        $sheet->setCellValue('D14', $this->translator->trans('keyword_descriptions'));
        ...
        $sheet->setCellValue('H14', $this->translator->trans('keyword_recipes'));
        ExcelUtilities::setFont($sheet->getCell('H14'), 11, true);
        ExcelUtilities::setCellAlignment($sheet->getCell("H14"), $alignmentH);
        ExcelUtilities::setVerticalCellAlignment($sheet->getCell("H14"), $alignmentV);
        ExcelUtilities::setBackgroundColor($sheet->getCell("H14"), "ECECEC");

        $sheet->mergeCells("J14:K14");
        $sheet->setCellValue('J14', $this->translator->trans('keyword_expenses'));
        ExcelUtilities::setFont($sheet->getCell('J14'), 11, true);
        ExcelUtilities::setCellAlignment($sheet->getCell("J14"), $alignmentH);
        ExcelUtilities::setVerticalCellAlignment($sheet->getCell("J14"), $alignmentV);
        ExcelUtilities::setBackgroundColor($sheet->getCell("J14"), "ECECEC");
        ...
        foreach ($result as $key => $line) {
            if ($line['closure']) {
                ...
                foreach ($line['expensesBySubGroup'] as $expense) {
                    $label = $this->getLabelExpense($expense['groupExpense'], $expense['subGroup']);
                    $sheet->mergeCells("B".$indexCell.":C".$indexCell);
                    $sheet->setCellValue('B'.$indexCell, $key.' - '.$index);
                    if ($index == 1) {
                        ExcelUtilities::setBackgroundColor($sheet->getCell('B'.$indexCell), "CA9E67");
                    }
                    $sheet->mergeCells("D".$indexCell.":G".$indexCell);
                    $sheet->setCellValue('D'.$indexCell, $label);
                    $sheet->mergeCells("H".$indexCell.":I".$indexCell);
                    $sheet->mergeCells("J".$indexCell.":K".$indexCell);
                    $sheet->setCellValue('J'.$indexCell, number_format($expense['totalAmount'], 2, ',', ''));
                    $index++;
                    $indexCell++;
                }
                ...
                if ($line['chestGap'] < 0) {
                    ...
                    $sheet->setCellValue('B'.$indexCell, $key.' - '.$index);
                    $sheet->mergeCells("D".$indexCell.":G".$indexCell);
                    $sheet->setCellValue('D'.$indexCell, $this->translator->trans('keyword.chest_gap'));
                    $sheet->mergeCells("H".$indexCell.":I".$indexCell);
                    $sheet->mergeCells("J".$indexCell.":K".$indexCell);
                    $sheet->setCellValue('J'.$indexCell, abs(number_format($line['chestGap'], 2, ',', '')));
                    $index++;
                    $indexCell++;
                }
                ...
                foreach ($line['recipeByLabel'] as $recipeTicket) {
                    ...
                    $sheet->mergeCells("H".$indexCell.":I".$indexCell);
                    $sheet->mergeCells("J".$indexCell.":K".$indexCell);
                    $sheet->setCellValue('H'.$indexCell, number_format($recipeTicket['totalAmount'], 2, ',', ''));
                    $index++;
                    $indexCell++;
                }
                if ($line['chestGap'] > 0) {
                    ...
                    $sheet->mergeCells("H".$indexCell.":I".$indexCell);
                    $sheet->mergeCells("J".$indexCell.":K".$indexCell);
                    $sheet->setCellValue('H'.$indexCell, number_format($line['chestGap'], 2, ',', ''));
                    $index++;
                    $indexCell++;
                }
                ...
                $sheet->mergeCells("D".$indexCell.":G".$indexCell);
                $sheet->setCellValue('D'.$indexCell, $this->translator->trans('cash_book.daily_recipe'));
                $sheet->mergeCells("H".$indexCell.":I".$indexCell);
                $sheet->mergeCells("J".$indexCell.":K".$indexCell);
                $sheet->setCellValue('H'.$indexCell, number_format($line['dailyRecipe'], 2, ',', ''));
                $index++;
                $indexCell++;
                ...
                $sheet->mergeCells("D".$indexCell.":G".$indexCell);
                $sheet->setCellValue('D'.$indexCell, $this->translator->trans('keyword_balancing'));
                $sheet->mergeCells("H".$indexCell.":I".$indexCell);
                $sheet->mergeCells("J".$indexCell.":K".$indexCell);
                $sheet->setCellValue('H'.$indexCell, number_format($line['currentBalancing'], 2, ',', ''));
                $index++;
                $indexCell++;
                ...
                $sheet->mergeCells("D".$indexCell.":G".$indexCell);
                $sheet->setCellValue('D'.$indexCell, $this->translator->trans('keyword_in_chest'));
                $sheet->mergeCells("H".$indexCell.":I".$indexCell);
                $sheet->mergeCells("J".$indexCell.":K".$indexCell);
                $sheet->setCellValue('H'.$indexCell, number_format($line['currentChestAmount'], 2, ',', ''));
                $indexCell++;
            }
        }
        //Border
        for ($cp = 14; $cp < $indexCell; $cp++) {
            ...
            ExcelUtilities::setBorder($sheet->getCell('F'.$cp));
            ExcelUtilities::setBorder($sheet->getCell('G'.$cp));
            ExcelUtilities::setBorder($sheet->getCell('H'.$cp));
            ExcelUtilities::setBorder($sheet->getCell('I'.$cp));
            ExcelUtilities::setBorder($sheet->getCell('J'.$cp));
            ExcelUtilities::setBorder($sheet->getCell('K'.$cp));
        }
