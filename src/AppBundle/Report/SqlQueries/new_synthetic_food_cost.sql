  ...
  (
    ...
    FROM
      ...
      (
        ...
        WHERE
          lower(tp.type) like lower(:discount_label_type)
      ...
      (
        ...
        FROM ticket t2 JOIN ticket_payment tp ON t2.id = tp.ticket_id
        WHERE
          tp.id_payment like :bon_repas_label_type
          AND t2.status NOT IN (-1, 5) AND t2.counted_canceled <> TRUE
          AND t2.date >= :startDate
          AND t2.date <= :endDate
        GROUP BY t2.date
      ...
      (
        ...
        WHERE
          ...
          AND consumed_qty.date <= :endDate AND consumed_qty.date >= :startDate
      ...
      (
        ...
        WHERE
          ...
          AND ls.entry :: DATE <= :endDate AND ls.entry :: DATE >= :startDate
        GROUP BY ls.entry
      ...
      (
        ...
        WHERE
          ...
          AND ls.entry :: DATE >= :startDate AND ls.entry :: DATE <= :endDate
