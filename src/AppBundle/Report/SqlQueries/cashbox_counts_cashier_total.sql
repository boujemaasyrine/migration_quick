...
FROM
  ...
  (
	...
	LEFT JOIN ticket on TP.ticket_id = ticket.id
         ...
         AND ticket.type =:ticket_type AND ticket.status = -1  AND ticket.counted=true AND
          TP.id_payment in (:cb_ids)
          AND ticket.origin_restaurant_id = :origin_restaurant_id
 )S23,
  (
  ...
  ,
  (
    SELECT SUM(Qq.qty * Qq.unit_value) AS cq_real
    FROM
  ...
  ,
  (
    SELECT SUM(TP.amount) AS cq_theoretical
    FROM
      public.cashbox_check_quick_container Q
      LEFT JOIN public.cashbox_count C ON Q.cashbox_id = C.id
