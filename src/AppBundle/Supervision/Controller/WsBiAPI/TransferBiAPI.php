 ...
 * @Route("/ws_bi_api")
 */
class TransferBiAPI extends Controller
{

    /**
     * @Route("/transfer_lines")
     *
     * @param Request $request
     * @return JsonResponse
     */
    public function transferLinesAction(Request $request)
    {
        try {
            ...
            $file = $this->get('bi_api.response.service')
                ->generateCsvFile('bi_api.transfer.service', 'getTransferLines',
                    ['criteria' => [
                        'startDate' => $startDate,
                        'endDate' => $endDate,
            ...
            return $response = Utilities::createCsvFileResponse($file, date('Ymd') . "C." . $restaurantId);
        } catch (Exception $e) {
            return $response = new JsonResponse([
