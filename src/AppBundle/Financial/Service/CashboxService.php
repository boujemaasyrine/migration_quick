...
class CashboxService
{
    ...
    )
    {
        ...
        if (!is_null($cashboxCount->getCashier())) {
            ...
            /**
             * Pour calculer totale théorique et le totale amount de discount container
             * @var CashboxDiscountContainerService $cdcs
             */
            $cdcs = $this->container->get("cashbox.discount.container.service");
            $cashboxDiscount = $cashboxCount->getDiscountContainer();
			$cashboxDiscount->setOriginRestaurantId($restaurant->getId());
            $cashboxDiscount->setDiscountLabels(
                $cdcs->listDiscountLabes($cashboxDiscount,true)
            );
            $cashboxDiscount->setAmountByLabelsArray(
                $cdcs->generateAmountByLabels($cashboxDiscount,true)
            );
            $cashboxDiscount->setQuantityByLabelsArray(
                $cdcs->generateQuantityByLabels($cashboxDiscount,true)
            );
            $cashboxDiscount->setTotalQuantity(
                $cdcs->generateTotalQuantity($cashboxDiscount,true)
            );
            $cashboxDiscount->setTheoricalTotal(
                $cdcs->calculateTheoricalTotal($cashboxDiscount,true)
            );
            $cashboxDiscount->setTotalAmount(
                $cdcs->calculateTotalAmount($cashboxDiscount,true)
            );
        }
    }
    ...
    )
    {
        ...
        if (!is_null($cashboxCount->getCashier())) {
            ...
            //Pour calculer totale théorique electronic et non electronic
            /**
             * @var CashboxCheckRestaurantContainerService $ccrcs
             */
            $ccrcs = $this->container->get("cashbox.check.restaurant.container.service");
             ...
             */
            $cmtcs = $this->container->get("cashbox.meal.ticket.container.service");
    ...
    )
    {
        try {
            ...
            if (!$kiosk) {
                $globalCashCashboxCount
                    = $this->parameterService->getGlobalCashCashboxCountParameter();
                $globalCashCashboxCount->setValue(
                    $cashboxCount->getCashContainer()->isAllAmount()
                );
            }
    ...
    public function findCashboxCountsInDatesInterval(\DateTime $from, \DateTime $to, $restaurant = null)
    {
        ...
        return $this->em->getRepository('Financial:CashboxCount')
            ->findCashboxCountsInDatesInterval(
                $from,
                $to,
                $restaurant
     ...
     *
     */
    public function getNotCountedCashBoxCount(\DateTime $from, \DateTime $to)
    {
        $currentRestaurant = $this->restaurantService->getCurrentRestaurant();
        $qb = $this->em->getRepository("Financial:Ticket")->createQueryBuilder(
            't'
        );
        $qb->select('count(DISTINCT(t.operator))')
            ->where('t.counted = :counted')
            ->setParameter('counted', false)
