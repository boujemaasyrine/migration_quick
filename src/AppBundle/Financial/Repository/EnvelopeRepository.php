...
class EnvelopeRepository extends EntityRepository
{
    ...
    public function getEnvelopeToday($sousType, $restaurant)
    {
    ...
    public function getEnvelopeWithdrawalNotCounted($restaurant = null)
    {
        $qb = $this->createQueryBuilder('e');

        if (isset($restaurant)) {
            $qb->where("e.originRestaurant = :restaurant")
                ->andWhere('e.chestCount is null')
                ->andWhere('e.source = :withdrawal')
                ->setParameter('restaurant', $restaurant)
                ->setParameter('withdrawal', Envelope::WITHDRAWAL);
        }
        return $qb->getQuery()->getResult();
    }

    /**
     * retourne tous les enveloppes qui ont été crées après la dernièr cloture
     * @param null $restaurant
     * @param $date
     * @return array
     */
    public function getEnvelopeWithdrawalOfClosing($restaurant = null, $date)
    {
        $qb = $this->createQueryBuilder('e');
