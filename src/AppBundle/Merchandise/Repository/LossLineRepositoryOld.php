...
class LossLineRepository extends \Doctrine\ORM\EntityRepository
{
    ...
    public function getFiltredLossLine($filter, $onlyTotal = false)
    {
        ...
        $sql = "SELECT
                    ...
                    where LS.origin_restaurant_id = :origin_restaurant_id and P.active=true and LS.entry >= :D1 and LS.entry <= :D2 and LS.type = :article GROUP BY Entryday, P.id ORDER BY P.Name";
    ...
    public function getFiltredLossLineSold($filter, $onlyTotal = false)
    {
	    ...
	    LEFT JOIN public.product P on P.id = PS.id
            where LS.origin_restaurant_id = :origin_restaurant_id and LS.entry >= :D1 and LS.entry <= :D2 and LS.type = :final GROUP BY PS.id, Entryday ORDER BY productName
        ";
