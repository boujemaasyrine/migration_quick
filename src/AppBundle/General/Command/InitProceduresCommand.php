    ...
    protected function configure()
    {
        $this->setName('quick:procedure:init')->setDefinition([])
            ->addArgument(
                ...
                'the restaurant id for the restaurant to initialize'
            )
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (count($roles) == 0) {
            ...
            $this->logger->addAlert(
                'Please import the roles first.',
                ['InitProceduresCommand']
            );
        ...
        if ($restaurant == null) {
            ...
            $ouverture = $this->em->getRepository(Procedure::class)->findOneBy(
                array(
                    'name'             => 'ouverture',
            ...
            $ouverture->addNameTranslation('nl', 'Opening');
            ...
            foreach ($ouvertureActionNames as $key => $a) {
                ...
                if ($action) {
                    ...
                    $step->setAction($action)
                        ->setOrder($key + 1)
                        ->setDeletable(
                            ($ouvertureActionFixed[$key] == 0) ? true : false
                        )
            ...
            $fermeture = $this->em->getRepository(Procedure::class)->findOneBy(
                array(
                    'name'             => 'fermeture',
                    'originRestaurant' => $restaurant,
                )
            ...
            $fermeture->addNameTranslation('nl', 'Sluiting');
            $this->em->flush();
            ...
            foreach ($fermetureActionNames as $key => $a) {
                ...
                if ($action) {
                    ...
                    $step->setAction($action)
                        ->setOrder($key + 1)
                        ->setDeletable(
                            ($fermetureActionFixed[$key] == 0) ? true : false
                        )
