...
class ImportWyndCommand extends ContainerAwareCommand
{
    ...
    protected function initialize(InputInterface $input, OutputInterface $output)
    {
        ...
        $this->ignoreLogger= $this->getContainer()->get('monolog.logger.ignored_tickets');
        $this->revenuePricesService = $this->getContainer()->get('prices.revenues.service');
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $this->logger->addDebug('Importing tickets', ['ImportWyndCommand']);
        ...
        try {
            ...
            if ($currentRestaurant == null) {
                $this->logger->addInfo('Restaurant not found with id: '.$restaurantId, ['quick:wynd:rest:import']);
            ...
            $t1 = time();
            $this->logger->addDebug("Start at ".date('H:i:s '), ['ImportWyndCommand']);
            ...
            foreach ($ticketsList as $type => $tickets) {
                ...
                foreach ($tickets as $val) {
                    //$this->logger->addDebug("Proceed Ticket Number $key ", ['ImportWyndCommand']);

                    $this->logger->addDebug("Ticket array : \n".print_r($val, true), ['ImportWyndCommand']);

                    $this->logger->addDebug("Ticket ".$val[$type]['date'], ['ImportWyndCommand']);

                    $this->logger->addDebug(
                        "invoice number".sprintf('%.0f', $val[$type]['invoiceNumber']),
                        ['ImportWyndCommand']
                    );

                    $existingTicket = $this->em->getRepository('Financial:Ticket')->findOneBy(
                        [
                            'invoiceNumber' => $val[$type]['invoiceNumber'],
                            'date' => new \DateTime($val[$type]['date']),
                    ...
                    if (is_null($existingTicket)) {
                        ...
                        if(intval($val[$type]['operator'])===0){
                            $this->ignoreLogger->addDebug('ticket ignored for 0 operator',['importWyndCommand']);
                        ...
                        elseif (empty($val['payments']) && floatval(
                                $val[$type]['total_ttc']
                            ) == 0 && !empty($val['lines']) && (count(
                                    $val['lines']
                                ) != count(
                                    $val[$type]['interventions']
                                )) && empty($val[$type]['discounts'])) {
                            $this->logger->addDebug('Ticket ignored for empty payment', ['ImportWyndCommand']);
                        } else {
                            if (empty($val['payments'])
                                ...
                                $this->ignoreLogger->addDebug('Ticket ignored: an empty payment with a total different from zero ',
                                    ['ImportWyndCommand']);
                                ...
                                $this->insertTicket($val, $type, $i, $array_tickets,$currentRestaurant);
                            }
            ...
            $this->em->clear();
            $t2 = time();
            $this->logger->addDebug("Finish at ".date('H:i:s '), ['ImportWyndCommand']);
            $this->logger->addDebug("Import finish in ".($t2 - $t1)." seconds", ['ImportWyndCommand']);
            $this->logger->addDebug($i." Tickets was imported.", ['ImportWyndCommand']);

            // if users are not synced then launch user import
            if (!$this->getContainer()->get('ticket.service')->isAllUsersAreSynced($currentRestaurant)) {
                try {
                    $this->getContainer()->get('staff.service')->importUsers($currentRestaurant);
                } catch (\Exception $e) {
                    $this->logger->addError(
                        'Users need to be synced but the staff import failed :'.$e->getMessage(),
                        ['ImportWyndCommand']
                    );
                    throw $e;
                }
    ...
    public function insertTicket($val, $type, $i, &$array_tickets, Restaurant $restaurant)
    {
        ...
        $ticket
            ->setType($type)
            ->setDate(($val[$type]['date'] != null) ? new \DateTime($val[$type]['date']) : null)
            ->setStartDate(
                ($val[$type]['date_ticket_start'] != null) ? new \DateTime($val[$type]['date_ticket_start']) : null
            )
            ->setEndDate(
                ($val[$type]['date_ticket_end'] != null) ? new \DateTime($val[$type]['date_ticket_end']) : null
            )
            ->setNum(0)
            ...
            ->setDeliveryTime(
                ($val[$type]['delivery_time'] != null) ? new \DateTime($val[$type]['delivery_time']) : null
            )
            ->setOperator(intval($val[$type]['operator']))
            ->setOperatorName(isset($val[$type]['operatorName']) ? $val[$type]['operatorName'] : null)
            ->setResponsible($val[$type]['responsable'])
            ->setWorkstation(intval($val[$type]['workstation']))
            ->setWorkstationName(isset($val[$type]['cashDeskID']) ? $val[$type]['cashDeskID'] : null)
            ->setOriginId(isset($val[$type]['origin_id']) ? intval($val[$type]['origin_id']) : null)
            ->setOrigin($val[$type]['origin'])
            ->setDestinationId(isset($val[$type]['destination']) ? intval($val[$type]['destination']) : null)
        ...
        if (floatval($val[$type]['total_ttc']) == 0 && empty($val['payments']) && (empty($val['lines']) || (count(
                        $val['lines']
            ...
            foreach ($val[$type]['interventions'] as $intKey => $int) {
                $amount += floatval($int['item']['price']) * intval($int['item']['quantity']);
            }
                    ...
                    foreach ($val['lines'] as $pLine => $l) {
                        ...
                        if ($l['discount_id'] == $employeeDiscountId) {
                            ...
                            $line
                                ...
                                ->setDivision(isset($l['division']) ? intval($l['division']) : 0)
                                ...
                                ->setTotalTVA(abs(floatval($l['discount_tva'])));
                            ...
                            $line
                                ...
                                ->setTotalTTC(floatval($l['total_TTC']) + abs(floatval($l['discount_ttc'])))
                                ->setCategory(isset($l['category']) ? $l['category'] : null)
                                ->setDivision(isset($l['division']) ? intval($l['division']) : 0)
                                ->setProduct($l['product'])
                                ->setLabel(isset($l['label']) ? $l['label'] : null)
                        ...
                        $revenuPrice = $this->revenuePricesService->calculateFinancialRevenueForTicketLine($line, $restaurant);
                        $line->setRevenuePrice($revenuPrice);
                        $ticket->addLine(clone $line);

                    }
                    $ticket->setTotalHT($totalHt + floatval($val[$type]['total_ht']));
                    $ticket->setTotalTTC($totalTTC + floatval($val[$type]['total_ttc']));
                }

                // add a payment block of type meal ticket
                foreach ($val[$type]['discounts'] as $pKey => $discount) {
                    if ($discount["id"] == $employeeDiscountId) {
                        $block = array("id" => 5, "code" => "BR", "label" => "Bon repas", "amount" => $discount["amount"], "employee" => $discount["label"]);
                        $val["payments"][] = $block;
        ...
        if (isset($val[$type]['interventions'])) {
            foreach ($val[$type]['interventions'] as $intKey => $int) {
                ...
                $intervention
                    ...
                    ->setDate(($int['date'] != null) ? new \DateTime($val[$type]['date']) : null)
                    ->setPostTotal($int['postTotal']);
                if ($intervention->getAction() === TicketIntervention::ABONDON_ACTION) {
                    $intervention->setItemAmount($int['item']['amount']);
                    $ticket->setCancelledFlag(true);
                } else {
                    if ($intervention->getAction() === TicketIntervention::DELETE_ACTION) {
                        $intervention
                        ...
                        if ($intervention->getAction() === TicketIntervention::DELETE_PAYMENT_ACTION) {
                            $intervention
                                ->setItemAmount($int['item']['amount'])
                                ->setItemCode($int['item']['code']);
                        } elseif ($intervention->getAction() === TicketIntervention::DECREASE_QUANTITY_ACTION) {
                            $intervention->setItemPLU($int['item']['external_ref'])
                                ->setItemId($int['item']['id'])
        ...
        if (isset($val['payments'])) {
            ...
            if (isset($val[$type]['invoiceCancelled']) && !is_null($val[$type]['invoiceCancelled'])) {
                $existingTicket = $this->em->getRepository('Financial:Ticket')->findOneBy(
                    [
                ...
                if (!is_null($existingTicket)) {
                    ...
                    $this->processTicketPayment($array_tickets[$val[$type]['invoiceCancelled']], $ticket);
                }
            }
        }
            ...
            if (sizeof($roundings) == 0) {
                ...
                foreach ($val[$type]['discounts'] as $pKey => $p) {
                    if ($p['code'] != 'ROUNDING') {
                        if (sizeof($roundings) != 0) {
                            $key = $roundings[0];
                            $rounding = $val[$type]['discounts'][$key]['amount'];
                            $payment = new TicketPayment();
                            $payment
                                ->setType(TicketPayment::DISCOUNT_TYPE)
                                ->setNum(intval($pKey))
                                ->setLabel($p['label'])
                                ->setAmount(floatval($p['amount']) + floatval($rounding));
                            $ticket->addPayment(clone $payment);
        ...
        $this->em->persist($ticket);
        $this->productPurchasedMvmtService->createMvmtEntryForTicket($ticket,$restaurant);
        if ($i % 50 === 0) {
            $this->em->flush();
        }
    }
     ...
     */
    private function removeTicket($numberInvoiceCanceledToBedeleted, $existingTicket)
    {
        ...
        $this->em->flush();
        $this->logger->addInfo("Canceled Ticket deleted with success :".$numberInvoiceCanceledToBedeleted);
    }
    ...
    private function processTicketPayment($val, &$ticket)
    {
        foreach ($val['payments'] as $pKey => $p) {
            ...
            if (isset($p['id'])) {
                    ...
                    if($p['id']==strval(TicketPayment::EDENRED)||$p['id']==strval(TicketPayment::E_SODEXO)||$p['id']==strval(TicketPayment::PAYFAIR)){
