...
class ServerDownEmailCommand extends ContainerAwareCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        try {
            $mail = \Swift_Message::newInstance()
                ->setSubject("[QUICK] MAIN SERVER IS DOWN")
                ->setFrom(array($fromMail))
                ->setTo(array($mail))
                ->setBody(
                    "Main server is down. We are working on starting backup server on this address $address..",
                    'text/html'
                );
