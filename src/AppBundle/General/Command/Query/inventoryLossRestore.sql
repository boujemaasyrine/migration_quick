...
format('SELECT  *  FROM public.loss_sheet where type=''article'' and entry <= %L and entry >= %L and origin_restaurant_id=%L ',:d2::timestamp  ,:d1::timestamp ,:restaurantId::int) )
AS t(id integer, employee_id integer, model_id integer, origin_restaurant_id integer, type character varying(255), status character varying(10),
 ...
 origin_restaurant_id=:restaurantId and entry <= :d2 and entry >= :d1 and type='article' );
INSERT INTO public.loss_line SELECT * FROM dblink ('dbname =backupdb port =8080 host =localhost user =postgres password =123456',
format('SELECT  *  FROM public.loss_line where loss_sheet_id in ( select id from loss_sheet where entry <= %L and entry >= %L and origin_restaurant_id=%L and type=''article'') ',:d2::timestamp ,:d1::timestamp  ,:restaurantId::int) )
AS t(id integer,loss_sheet_id integer,product_id integer,  recipe_id integer, recipe_historic_id integer,
 ...
 ( select id from loss_sheet where entry <= :d2 and entry >= :d1 and origin_restaurant_id=:restaurantId and type='article'));
...
format('SELECT  *  FROM public.product_purchased_mvmt where   type = (''purchased_loss'') and  date_time <= %L and date_time >= %L and  origin_restaurant_id=%L',:d2::timestamp ,:d1::timestamp ,:restaurantId::int ) )
...
source_id numeric ,stock_qty double precision , type character varying (50), buying_cost double precision , label_unit_exped character varying(255),
 ...
 WHERE id NOT IN (SELECT id FROM public.product_purchased_mvmt  WHERE  date_time <= :d2 and date_time >= :d1  and type = ('purchased_loss') and  origin_restaurant_id=:restaurantId )
